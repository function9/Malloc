//Partners Voravee Upatising and Jacqueline Thornton
/*-------------------------------------------------------------------                             
 * Lab 5 Starter code:                                                                            
...
void* mm_malloc (size_t size) {
  ...
  //from textbook, if no fit is found, request more memory and place block
  ...
  //prevents overlapping by removing indetified block from free list
  removeFreeBlock(ptrFreeBlock);
  //predefining variables used in splitting the blocks (if needed)
  BlockInfo* followingBlock;
  size_t size1 = (blockSize - reqSize);
  size_t size2 = (reqSize - WORD_SIZE);
  size_t size3 = (blockSize - WORD_SIZE);
   //splits blocks when size1 is less than MIN_BLOCK_SIZE
   if(size1 < MIN_BLOCK_SIZE){
    *((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, size3)) = blockSize;
    //sets the next block's previous tag to used
    followingBlock = (BlockInfo *) UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize);
    followingBlock->sizeAndTags = TAG_PRECEDING_USED | followingBlock->sizeAndTags;
   }
   else{
    //changing the header to appropriate location
    *((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, size2)) = reqSize;
    //updating the tags to used
    ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag;
    //updating information of the remaining block in memory
    followingBlock = (BlockInfo *) UNSCALED_POINTER_ADD(ptrFreeBlock,reqSize);
    followingBlock->sizeAndTags = (size1 | TAG_PRECEDING_USED); 
    followingBlock->sizeAndTags &= (~TAG_USED);
    insertFreeBlock(followingBlock);
    //changing the length of the remaining block in memory
    *((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, size3)) = size1;
  }
  //setting blocks tag to used
